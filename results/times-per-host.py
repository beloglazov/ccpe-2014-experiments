...
for hostname, host_id in db.select_host_ids().items():
    ...
    print '  Overload time: ' + \
        str(overload_time)
    print '  Non-overload time: ' + \
        str(nonoverload_time)
    print '  Total time: ' + \
        str(nonoverload_time + overload_time)
    print '  Idle time: ' + \
        str(off_time)
    print '  Non-idle time: ' + \
        str(nonoverload_time + overload_time - off_time)
    print '  ITF: ' + \
        str(float(off_time) / (nonoverload_time + overload_time))
    print '  OTF: ' + \
        str(float(overload_time) / (nonoverload_time + overload_time - off_time))
...
print 'AITF: ' + str(float(total_idle_time) / total_time)
print 'AOTF: ' + str(float(total_overload_time) / (total_time - total_idle_time))
